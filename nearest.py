...
from helpers import API_PARAMS, read_pickle_all_stops, pickle_metro_rail_stops, load_cached_stops, CachedStop
...
def nearest_stops_request(
    ...
    try:
        ...
        return [Stop.init_from_dict(s) for s in res_stops["stopPoints"]]
...
def nearest_stops_cached(
    ...
    lng: str,
    radius: int = 3000,
    stop_types: list[str] = ["NaptanMetroStation", "NaptanRailStation"],
    # modes: str = "tube,dlr,overground,elizabeth-line,bus",
    # categories: str = "none",
    num_stops: int = 10
) -> list[Stop]:
    dist_to_stop = lambda s: haversine((float(lat), float(lng)), (s.lat, s.lon), unit=Unit.METERS)
    cached_stops = load_cached_stops()
    cached_stops_by_dist = sorted(cached_stops, key=dist_to_stop)
    top_n_stops = [Stop.init_from_cached(c, distance=dist_to_stop(c)) for c in cached_stops_by_dist[:num_stops]]
